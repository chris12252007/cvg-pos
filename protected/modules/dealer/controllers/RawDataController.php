...
class RawDataController extends DealerController
{
    /**
    ...
    public function accessRules()
    {
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update', 'delete', 'admin', 'view', 'index', 'syncRecords'),
                'users' => array('@'),
            ),
...
// $this->performAjaxValidation($model);
        ...
        if (isset($_POST['RawData'])) {
            ...
            if ($model->validate()) {
                $model->save();
                Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'New RawData Successfully Created.');
...
// $this->performAjaxValidation($model);
        ...
        if (isset($_POST['RawData'])) {
            ...
            if ($model->validate()) {
                $model->save();
                Utilities::set_Flash(Utilities::FLASH_SUCCESS, 'RawData Successfully Updated');
                $this->redirect(array('view', 'id' => $model->id));
            } else {
                Utilities::set_Flash(Utilities::FLASH_ERROR, Utilities::get_ModelErrors($model->errors));
            }
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
    ...
    public function actionAdmin()
    {
        ...
        Utilities::setMenuActive_Siteadmin(Settings::get_ControllerID(), 'RawData::tbl()', Settings::get_ActionID());
    ...
    public function loadModel()
    {
        if ($this->_model === null) {
            if (isset($_GET['id']))
                $this->_model = RawData::model()->findbyPk($_GET['id']);
            if ($this->_model === null)
    ...
    {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'raw-data-form') {
    ...
    {
        $this->redirect($this->createUrl('RawData/create'));
    }

    public function actionSyncRecords()
    {
        $model = new RawData();
        $model = RawData::sql_getAllData();
//        $datTest  =array();
//        $dataPoint = array();
...
//            {
//                $datTest = array($cust['customer_name'] => $modules);
//            }
